...
class CustomLandmarkDataset(Dataset):
    def __init__(self, folder_labels, digits, dataset_id, transform=None):
        ...
        self.preprocessed_file = (
            ...
            + ".pt"
            if digits
            else "data/preprocessed/letters/preprocessed_data" + str(dataset_id) + ".pt"
        )
        self.digits = digits
...
folder_labels0 = [
    (f"data/raw/dataset0/{chr(i)}", chr(i)) for i in range(ord("A"), ord("Z") + 1)
]
folder_labels1 = [
    (f"data/raw/dataset1/{chr(i)}", chr(i)) for i in range(ord("A"), ord("Z") + 1)
]
folder_labels2 = [
    (f"data/raw/dataset2/{chr(i)}", chr(i)) for i in range(ord("A"), ord("Z") + 1)
]
folder_labels3 = [
    (f"data/raw/dataset3/{chr(i)}", chr(i)) for i in range(ord("A"), ord("Z") + 1)
]
folder_labels4 = [
    (f"data/raw/dataset4/{chr(i)}", chr(i)) for i in range(ord("A"), ord("Z") + 1)
]
...
# Définition de la proportion de données pour le test
...
# Création des DataLoaders pour pouvoir charger le dataset en batches (utile pour le réseau de neurones ensuite).
